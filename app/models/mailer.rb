...
class Mailer < ActionMailer::Base
  ...
  def issue_add(issue)
    ...
    priority = issue.priority.nil? ? l(:label_none) : issue.priority.name
    assigned_to = issue.assigned_to.nil? ? l(:label_none) : issue.assigned_to.name
    subject "[#{issue.project.name} - #{issue.tracker.name} - #{l(:field_priority)}:#{priority} - #{l(:field_assigned_to)}:#{assigned_to} ##{issue.id}] (#{issue.status.name}) #{issue.subject}"
    body :issue => issue,
  ...
  def issue_edit(journal)
    ...
    priority = issue.priority.nil? ? l(:label_none) : issue.priority.name
    assigned_to = issue.assigned_to.nil? ? l(:label_none) : issue.assigned_to.name
    s = "[#{issue.project.name} - #{issue.tracker.name} - #{l(:field_priority)}:#{priority} - #{l(:field_assigned_to)}:#{assigned_to} ##{issue.id}] "

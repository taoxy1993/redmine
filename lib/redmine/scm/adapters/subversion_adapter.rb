    ...
    module Adapters    
      class SubversionAdapter < AbstractAdapter
        ...
        def entries(path=nil, identifier=nil)
          ...
          @findproject = Repository.find(:all)
          @findproject.each do |findproject|
            a = findproject.url+"/"
            check = target(path).index(a)
            if(check == 1)
              @b = findproject.project_id
              break
            end
          end
          ...
          shellout(cmd) do |io|
            ...
            begin
              ...
              each_xml_element(doc['lists']['list'], 'entry') do |entry|
                ...
                @nowpath = (path.empty? ? "" : "#{path}/") + name
                 
                @svnmanage = Svnmanage.find(:all,:conditions=>"login = '#{@user.login}' and project_id = '#{@b}'")
                @c = "nosuper"
                ...
                if(@c == "nosuper")                                  
                  @svnmanage.each do |svnmanage|
                    d = @nowpath.index(svnmanage.svnpath)
                    if(d == 0)
                      @c = "super"
                      break
                    end    
                    d = svnmanage.svnpath.index(@nowpath)
                    if(d == 0)
                      @c = "super"
                      break
                    end    

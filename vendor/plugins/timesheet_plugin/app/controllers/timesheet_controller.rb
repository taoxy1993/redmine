class TimesheetController < ApplicationController
  ...
  def department_report
    ...
    if params && params[:timesheet]
      ...
      if params[:department_id]
        ...
        if @department.is_manager(User.current)
          @department.all_members.each do |u|
            user_ids << u.id
          end
        else
          flash[:error] = "你没有权限查询[部门 - #{@department.to_s}]工时登记情况！"
          redirect_to :action => 'department_index'
          return
        end

        @timesheet.users = user_ids

        puts ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#{@department.name}-#{@department.manager}-#{@department.sub_manager}"
      end
    else
    # 响应出department_index接口
      redirect_to :action => 'department_index'
      return
    end

    @timesheet.allowed_projects = department_allowed_projects
    ...
    save_filters_to_session(@timesheet)
    puts "============before================>:#{@timesheet.time_entries.size}"
    @timesheet.fetch_time_entries_for_department
    puts "==============after==============>:#{@timesheet.time_entries.size}"
  ...
  def department_allowed_projects
    Project.timesheet_order_by_name
  end
